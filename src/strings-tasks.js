...
function isPalindrome(str) {
  const strTrimmed = str.toLocaleLowerCase().replace(/[\W]/g, '');
  const strTrimmedReverse = strTrimmed.split('').reverse().join('');
  if (strTrimmed === strTrimmedReverse) {
    return true;
  }
  return false;
}
...
function encodeToRot13(str) {
  const defaultAlp = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const rot13Alp = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  const result = str.replace(/[a-z]/gi, (l) => rot13Alp[defaultAlp.indexOf(l)]);
  return result;
}
